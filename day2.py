...
def parse_input(input_):
    games = {}
    for line in input_:
        rexp1 = re.compile("Game ([0-9]*)\: (.*)")
        rexp2 = re.compile("\s*([0-9]*) ([a-z]*)")
        m = rexp1.match(line)
        game_id = int(m.group(1),10)
        ...
        for r in m.group(2).split(';'):
            ...
            for g in r.split(','):
                m = rexp2.match(g)
                record[m.group(2)] = int(m.group(1),10)
...
def exo1(cubes_dict):
    ...
    for game in games.keys():
        possible = True
        for record in games[game]:
            for color in cubes_dict.keys():
                if cubes_dict[color] < record[color]:
                    possible = False
        if possible:
            print(game)
            sum += game
...
def exo2(cubes_dict):
    ...
    for game in games.keys():
        ...
        for record in games[game]:
            for color in cubes_dict.keys():
                if min_number[color] < record[color]:
                    min_number[color] = record[color]
        sum += min_number['red'] * min_number['green'] *min_number['blue']

    return (sum)

def main():
    cubes_dict = {'red':12,'green':13,'blue':14}
    print(" solu exo1 = "+str(exo1(cubes_dict)))
    print(" solu exo2 = "+str(exo2(cubes_dict)))
