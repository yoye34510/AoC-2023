...
class Number:
    ...
    def __str__(self):
        return("Number {} - row {} start_pos {} end_pos {}".format(self._value,self._row,self._start_pos,self._end_pos))
...
class Symbol:
    ...
    def __str__(self):
        return("Symbol {} - row {} pos {} ".format(self._value,self._row,self._pos))
...
def parse_input(input_):
    ...
    for row in input_:
        ...
        while col < len(row):
            number = ''
            scol   = col
            while col < len(row) and row[col].isdigit():
                number += row[col]
                col    += 1
            if number != '':
                numbers[row_num].append(Number(row_num,scol,col,number))
            elif row[col] != '.':
                symbols.append(Symbol(row_num,col,row[col]))
                col += 1
            else:
                col += 1
        row_num += 1
...
def exo1(numbers,symbols):
    res = 0
    for s in symbols:
        for n in numbers[s._row-1:s._row+2]:
            for a in n:
                if ( s._pos >= a._start_pos-1 ) and ( s._pos < a._end_pos+1 ):
                    res += int(a._value,10)
                    a._is_pn = True
    print(res)

def exo2(numbers,symbols):
    res = 0
    for g in [ gear for gear in symbols if gear._value == '*' ]:
        lst = []
        for n in numbers[g._row-1:g._row+2]:
            for a in n:
                if ( g._pos >= a._start_pos-1 ) and ( g._pos < a._end_pos+1 ) and (a._is_pn):
                    lst.append(int(a._value,10))
        if len(lst) == 2:
            res += lst[0] * lst[1]
    print(res)

def main():
    (n,s) = parse_input(input.split('\n'))
    exo1(n,s)
    exo2(n,s)
